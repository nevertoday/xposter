...
4. 打开 xPoster 侧边栏。
5. 点击 **编辑草稿** 粘贴 Markdown，或者选择 `.md` 文件。
   如果一次选择或拖入多篇 Markdown，xPoster 会把它们放进待发布队列，不会把所有正文直接铺在页面里。
6. 看预览和问题列表。
7. 点击 **Check article / 检查文章**。
8. 选中要写入的队列草稿，需要修改时在弹出框里编辑，然后点击 **Write article / Import / 写入文章 / 导入**。
9. 回到 X 文章编辑器里仔细检查。
10. 确认没问题后，再手动点击 X 自己的发布按钮。

## 批量草稿队列

一次加载多篇 `.md` 文件时，xPoster 会显示紧凑的待发布队列。每一行都可以载入、在弹出框里编辑、复制或移除。长篇 Markdown 不会直接展开在列表里，所以几十篇、上百篇草稿也更容易操作。

**当前草稿** 卡片会显示下一篇将要写入 X 的草稿。点击 **编辑草稿** 可以在弹出框里粘贴或修改 Markdown；弹出框打开时，后面的列表不会继续滚动。
